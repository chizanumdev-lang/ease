...
async function auditUsers() {
  ...
  try {
    ...
    if (!columns.includes('streak'))
      missingColumns.push('streak INTEGER DEFAULT 0');
    if (!columns.includes('level'))
      missingColumns.push('level INTEGER DEFAULT 1');
    if (!columns.includes('xp')) missingColumns.push('xp INTEGER DEFAULT 0');
    if (!columns.includes('is_admin'))
      missingColumns.push('is_admin BOOLEAN DEFAULT false');
    if (!columns.includes('is_verified'))
      missingColumns.push('is_verified BOOLEAN DEFAULT false');
