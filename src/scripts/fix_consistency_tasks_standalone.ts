...
async function bootstrap() {
  ...
  try {
    ...
    const programs = await dataSource.query(
      `SELECT id, user_id, duration, created_at FROM programs`,
    );
    ...
    for (const program of programs) {
      ...
      const dayPlans = await dataSource.query(
        `SELECT id FROM day_plans WHERE program_id = $1 AND day_number = $2`,
      ...
      const progress = await dataSource.query(
        `SELECT checkin_date FROM progress WHERE user_id = $1 ORDER BY checkin_date DESC LIMIT 30`,
        [program.user_id],
      );
      let streak = 0;
      ...
      await dataSource.query(
        `UPDATE tasks SET "order" = 3 WHERE day_plan_id = $1 AND (type = 'journal' OR title ILIKE '%journal%')`,
        [dayPlanId],
      );
      // Reflection
      await dataSource.query(
        `UPDATE tasks SET "order" = 4 WHERE day_plan_id = $1 AND (type = 'reflection' OR title ILIKE '%reflection%')`,
        [dayPlanId],
      );
      // Consistency
      const commitmentMsg = `i will complete my routine tommorrow. this will be day ${streak + 1} of my streak.`;
      await dataSource.query(
        `UPDATE tasks SET "order" = 5, description = $1 WHERE day_plan_id = $2 AND (type = 'consistency' OR title ILIKE '%commitment%' OR title ILIKE '%consistency%')`,
        [commitmentMsg, dayPlanId],
      );
