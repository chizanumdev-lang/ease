...
const shards = JSON.parse(
  fs.readFileSync('research/output/task-shards.json', 'utf8'),
);
...
async function run() {
  ...
  console.log(
    `Heuristics categorized ${results.length} shards. ${ambiguous.length} remaining for AI.`,
  );
  ...
  fs.writeFileSync(
    'research/output/task-shards-enriched.json',
