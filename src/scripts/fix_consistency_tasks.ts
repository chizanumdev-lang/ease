...
async function bootstrap() {
  ...
  const progressRepo = app.get<Repository<Progress>>(
  ...
  for (const program of programs) {
    console.log(
      `Processing program: ${program.title} (${program.id}) for user ${program.userId}`,
    );
    ...
    for (const task of tasks) {
      if (task.type === 'journal') {
        ...
        task.title.toLowerCase().includes('commitment')
      ) {
        task.order = 5;
