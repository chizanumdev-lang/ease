...
async function run() {
    ...
    try {
        ...
        await client.query('CREATE INDEX IF NOT EXISTS "IDX_day_plans_program_id" ON day_plans (program_id)');
        await client.query('CREATE INDEX IF NOT EXISTS "IDX_tasks_day_plan_id" ON tasks (day_plan_id)');
        ...
        const progRes = await client.query('SELECT id FROM programs WHERE user_id = $1', [userId]);
        ...
        if (planRes.rows.length === 0) {
            ...
            const tasksRes = await client.query('SELECT id, title FROM tasks WHERE day_plan_id = $1', [planId]);
            console.log(`Plan day 1 loaded. Tasks: ${tasksRes.rows.length} (${Date.now() - planStart}ms)`);
        }
