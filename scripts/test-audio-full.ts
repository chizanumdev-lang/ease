...
async function testFullAudioGen() {
    ...
    console.log(`Script length: ${script.length} characters (~${Math.round(script.split(' ').length)} words)\n`);
    ...
    return new Promise((resolve, reject) => {
        ...
        command
            .complexFilter([
                ...
                fs.existsSync(backgroundPath) ? '[voice][bg]amix=inputs=2:duration=first[a]' : '[voice]copy[a]'
            ].filter(f => f !== ''))
            .map('[a]')
