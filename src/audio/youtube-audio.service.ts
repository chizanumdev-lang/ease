...
export class YoutubeAudioService {
    ...
    constructor(private configService: ConfigService) {
        ...
        try {
            ...
            this.logger.error('Lazy loading FFmpeg failed in YoutubeAudioService', err);
        }
    }
    ...
    async extractAudio(videoId: string): Promise<string> {
        ...
        if (!apiKey || apiKey === 'your_rapidapi_key_here') {
            this.logger.warn(`RAPID_API_KEY is not configured. Falling back to library extraction (Unstable).`);
            throw new Error('RapidAPI Key missing. Please configure RAPID_API_KEY for reliable YouTube extraction.');
        }
        ...
        try {
            ...
            if (response.data.status !== 'ok' || !response.data.link) {
                this.logger.error(`RapidAPI conversion failed: ${JSON.stringify(response.data)}`);
                throw new Error(response.data.msg || 'Failed to get download link from RapidAPI');
            }
            ...
            this.logger.error(`Failed to extract audio for ${videoId} using RapidAPI`, error);
