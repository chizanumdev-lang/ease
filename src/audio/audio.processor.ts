...
export class AudioProcessor extends WorkerHost implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    try {
      const stuckTracks = await this.audioTrackRepository.find({
        where: [{ url: Like('%static_binaural%') }],
      });

      if (stuckTracks.length === 0) return;

      this.logger.log(
        `Found ${stuckTracks.length} stuck audio track(s) — re-queuing generation`,
      );
      ...
      for (const track of stuckTracks) {
        ...
        await this.audioQueue.add(
          'generate-audio',
          {
            ...
            theme,
            audioFilename: `program_track_${track.id}`,
          },
          { attempts: 3, backoff: { type: 'exponential', delay: 5000 } },
        );
        this.logger.log(
          `Re-queued audio generation for track ${track.id} (theme: ${theme})`,
        );
      }
    } catch (err) {
      this.logger.warn(
        `Failed to re-queue stuck audio tracks: ${err?.message}`,
      );
    }
  }
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    if (this.isVercel) {
      this.logger.warn(
        `Skipping audio job ${job.id} on Vercel serverless — needs dedicated worker`,
      );
      return { success: false, reason: 'serverless_skip' };
    }
    const { audioTrackId, theme, audioFilename } = job.data;
    this.logger.log(
      `Processing audio generation job ${job.id} for track ${audioTrackId}`,
    );
    ...
    try {
      ...
      if (
        ...
        this.logger.log(
          `Audio track ${audioTrackId} already generated, skipping job`,
        );
      ...
      if (track) {
        ...
        this.logger.warn(
          `AudioTrack ${audioTrackId} not found in DB after generation`,
        );
      }
      ...
      this.logger.error(
        `Failed to process audio generation job ${job.id}:`,
        error,
      );
      throw error;
    }
