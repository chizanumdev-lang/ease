...
export class AdminService {
  ...
  async getQueueStats() {
    ...
    return {
      ...
      delayed: 0,
      total: 0,
      note: 'Background tasks managed by Trigger.dev',
    };
  }
  ...
  async retryDayHydration(dayPlanId: string) {
    ...
    if (handle) {
      return {
        success: true,
        message: `Day ${day.dayNumber} sent to Trigger.dev for hydration.`,
      };
    } else {
      return {
        success: false,
        message: `Trigger.dev unavailable — Day ${day.dayNumber} status reset to pending but could not be queued.`,
      };
    }
  }
